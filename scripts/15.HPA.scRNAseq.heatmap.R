...
all <- rna_single_cell_type_tissue %>% filter(Tissue %in% c("lung", "pbmc")) %>% filter(`Gene name` %in% c("ATP11A", "DPP9", "NPNT", "MUC1", "OAS1", "PMF1"))
...
png("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-sQTLMR/results/Fig4B.png",width=900, height=600)
...
all <- normal_tissue %>% filter(`Gene name` %in% c("ATP11A", "DPP9", "NPNT", "MUC1", "OAS1", "PMF1"))
...
all <- consensus_tissue %>% filter(`Gene name` %in% c("ATP11A", "DPP9", "NPNT", "MUC1", "OAS1", "PMF1"))
