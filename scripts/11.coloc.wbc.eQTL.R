...
for(i in seq(1, dim(map)[1])){
  gene <- map$exposure[i]
  exp <- fread(paste0("eQTL_WBC/",gene,".tsv"))
  if(dim(exp)[1] >= 1){
...
write.table(result, file="A2.coloc.wbc.eQTL.tsv", append=F, sep="\t", col.names = T, row.names = F, quote=F)
...
colnames(result) <- c("phenotype_id", "NSNP","PP.H0.abf", "PP.H1.abf", "PP.H2.abf", "PP.H3.abf", "PP.H4.abf")
write.table(result, file="B2.coloc.wbc.eQTL.tsv", append=F, sep="\t", col.names = T, row.names = F, quote=F)
...
write.table(result, file="C2.coloc.wbc.eQTL.tsv", append=F, sep="\t", col.names = T, row.names = F, quote=F)
