...
# Lung <- Lung %>% mutate(pheno_id = paste0(str_split(exposure, pattern=":", simplify = T)[,1],":",
...
expression_Lung <- read.xlsx("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-sQTLMR/results/SuppleTable7.eQTL_Lung.xlsx")
expression_Lung <- expression_Lung %>% filter(PP.H4.abf > 0.8)

expression_WBC <- read.xlsx("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-sQTLMR/results/SuppleTable8.eQTL_WBC.xlsx")
expression_WBC <- expression_WBC %>% filter(PP.H4.abf  > 0.8)
...
Lung %>% mutate(outcome = fct_relevel(outcome, 
  ...
  geom_errorbar(aes(ymin=LL, ymax=UL, col=outcome), width=0.1, cex=1) + xlab("")  + ylab("Odds ratio (95%CI) per normalized intron excision ratios") + 
  ...
  scale_fill_manual(values=c("#00E6E6", "#FFD21A", "#FF9A00")) + theme(axis.text.x = element_text(size=20,face="bold"),
                                                           ...
                                                           strip.text = element_text(size=25)) +coord_flip() + guides(col = FALSE, fill = FALSE,
                                                                                                                      shape = FALSE)
ggsave("/home/richards/tomoko.nakanishi/my_project/repo/COVID19-sQTLMR/results/Fig2.png",width=12, height=12, dpi=300, bg = "white")
