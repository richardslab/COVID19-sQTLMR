...
sig <- read.xlsx("Lung.highcoloc.xlsx")
...
for(i in c(1,3,4,7,9,10,12,14,15,16,17)){
  ...
  write.table(DT_A2, file=paste0("sQTL_Lung/DT_A2.",sig$ensembleID[i],".txt"), quote=F, col.names = T, row.names = F, sep="\t")
}
...
for(i in c(1,3,4,7,9,10,12,14,15,16,17)){
  ...
  ld <- fread(paste0("LD/",sig$rsid[i],".ld")) %>% dplyr::select(SNP_B, R2)
  DT_A2 <- DT_A2 %>% inner_join(ld, by=c("SNP"="SNP_B"))
  write.table(DT_A2, file=paste0("sQTL_Lung/DT_B2.",sig$ensembleID[i],".txt"), quote=F, col.names = T, row.names = F, sep="\t")
}
...
for(i in c(1,3,4,7,9,10,12,14,15,16,17)){
  ...
  ld <- fread(paste0("LD/",sig$rsid[i],".ld")) %>% dplyr::select(SNP_B, R2)
  DT_A2 <- DT_A2 %>% inner_join(ld, by=c("SNP"="SNP_B"))
  write.table(DT_A2, file=paste0("sQTL_Lung/DT_C2.",sig$ensembleID[i],".txt"), quote=F, col.names = T, row.names = F, sep="\t")
}
