...
map = pd.read_csv(outpath+"sQTL_Lung.map", sep="\t")
for index, row in map.iterrows():
  dat = pd.read_parquet(exppath+"/Lung.v8.EUR.sqtl_allpairs."+row['CHR']+".parquet")
  select_dat = dat[dat['phenotype_id'].str.contains(row['group_id'])]
  select_dat.to_csv(outpath+"sQTL_Lung/"+row['group_id']+".tsv",sep="\t",index=False)
